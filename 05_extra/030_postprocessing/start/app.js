...
ポストプロセス
... その意味の通り、「 後処理 」を指す。
    この名前の由来は、通常のレンダリングプロセスが完了した後に行われる追加の処理を示すため。

    ・通常のレンダリング：
      →　レンダリングエンジン(例えばThree.js)は、シーンとカメラの情報を使って、3Dオブジェクトを2D画面に描画。
          このプロセスは「レンダリング」と呼ばれ、通常はこれだけで画面に映像が表示される
    ・ポストプロセス（後処理）：
      → 通常のレンダリングが完了した後、そのレンダリング結果に対して追加の処理が行われる。この追加の処理が「ポストプロセス」
...
→ 通常の3Dシーンのレンダリングが完了した後に、その描画結果に対して追加のエフェクトや
  ...
  シーンをカメラの視点からレンダリングしてフレームバッファに描画データを保存する。
  この際、直接ブラウザの画面に描画する

  EffectComposerを使用する場合↓
  通常のレンダリング結果を後でポストプロセスのエフェクトと合成するため、
  最初に通常のレンダリングによる描画データをRenderPassを通じてcomposerに追加する必要がある。
...
composer.addPass(renderPass);  // ここはテクスチャとして保持されない。
// → 以下のcomposerで追加していくデータはテクスチャとしてどんどん追加されていく。
...
// → glitchのエフェクトをテクスチャとして受けつぎ、新たなテクスチャを生成
// → composer.render()でディスプレイにレンダリング
...
/**************************************************************
レンダーターゲット ... 描画データを格納するバッファ(メモリ領域)のこと
...
function animate() {
  ...
  // ①レンダラーの描画データの保存先をレンダーターゲット(バッファ)に設定
  ...
  // ③レンダリングの描画データの保存先を通常のフレームバッファに戻す(リセット)
  ...
  // Composerをレンダリング(最終的なテクスチャをレンダリング)
